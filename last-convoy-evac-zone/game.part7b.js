...
function updateEvacConvoys(dt){
  ...
  for(const c of game.evacConvoys){if(c.done)continue;c.attackFlash=Math.max(0,c.attackFlash-dt);
...
function completeEvacConvoy(c){c.done=true;c.state='escaped';game.evacuated+=c.passengers;game.carried=Math.max(0,game.carried-c.passengers);toast('육상 대피 성공',`<strong>${c.passengers}명</strong>이 도시 봉쇄선을 통과했습니다.`);const q=c.group.position.clone();setTimeout(()=>{if(c.group.parent)scene.remove(c.group);},1200);c.shelter.state='evacuated';}
